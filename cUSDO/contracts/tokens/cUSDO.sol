 ...
 * @dev USDO Interface.
 */
interface IUSDO is IERC20MetadataUpgradeable {
    /**
    ...
    EIP712Upgradeable
{
    ...
    // cUSDO Errors
    error cUSDOBlockedSender(address sender);
    error cUSDOPausedTransfers();
    ...
    /**
     * @notice Initializes the ERC-4626 USDO Wrapper.
     * @param _USDO The address of the USDO token to wrap.
     * @param owner The owner address.
     */
    function initialize(IUSDO _USDO, address owner) external initializer {
        USDO = _USDO;

        __ERC20_init("Compounding Open Dollar", "cUSDO");
        __ERC4626_init(_USDO);
        __AccessControl_init();
        __Pausable_init();
        __UUPSUpgradeable_init();
        __EIP712_init("Compounding Open Dollar", "1");
     ...
     * @return Returns true if USDO or cUSDO is paused, and false otherwise.
     */
    function paused() public view override returns (bool) {
        return USDO.paused() || super.paused();
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (USDO.isBanned(from)) {
            revert cUSDOBlockedSender(from);
        }
