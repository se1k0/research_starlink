...
/*
 * Code based and adapted from Mountain Protocol
 * (Source: https://github.com/mountainprotocol/tokens as retrieved in May 2024)
 */
...
/**
 * @title OpenEden Protocol USDO Token Contract
 * @custom:security-contact support@OpenEden.com
 */
contract USDO is
    ...
    EIP712Upgradeable
{
    ...
    // Base value for bonusMultiplier
    uint256 private constant _BASE = 1e18;
    /**
     * @dev bonusMultiplier represents a coefficient used in bonus calculation logic.
    ...
    mapping(address => uint256) private _bannedList;
    ...
    error USDOInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
    error USDOInvalidBonusMultiplier(uint256 bonusMultiplier);
    error USDOBannedSender(address sender);
    error USDOInvalidBlockedAccount(address account);
    error USDOPausedTransfers();
    ...
    function convertToTokens(uint256 shares) public view returns (uint256) {
        return (shares * bonusMultiplier) / _BASE;
    }
    ...
    function _mint(address to, uint256 amount) private {
        if (to == address(0)) {
            revert USDOInvalidMintReceiver(to);
        }
    ...
    function _burn(address account, uint256 amount) private {
        if (account == address(0)) {
            revert USDOInvalidBurnSender(account);
        }
        ...
        if (accountShares < shares) {
            revert USDOInsufficientBurnBalance(account, accountShares, shares);
        }
    ...
    function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
        // Each banlist check is an SLOAD, which is gas intensive.
        // We only block sender not receiver, so we don't tax every user
        if (isBanned(from)) {
            revert USDOBannedSender(from);
        }
        // Useful for scenarios such as preventing trades until the end of an evaluation
        // period, or having an emergency switch for freezing all token transfers in the
        // event of a large bug.
        if (paused()) {
            revert USDOPausedTransfers();
        }
    }
     ...
     * @param account The address to banlist.
     *
     ...
     */
    function _bannedAccount(address account) private {
        if (isBanned(account)) {
    ...
    /**
     * @dev Private function that removes the specified address from the banlist.
     * @param account The address to remove from the banlist.
     */
    function _unbannedAccount(address account) private {
        if (!isBanned(account)) {
            revert USDOInvalidBlockedAccount(account);
        }
     ...
     * @dev This function can only be called by an account with BANLIST_ROLE.
     * @param addresses An array of addresses to be blocked.
     */
    function banAddresses(address[] calldata addresses) external onlyRole(BANLIST_ROLE) {
        for (uint256 i = 0; i < addresses.length; i++) {
            _bannedAccount(addresses[i]);
        }
    }

    /**
     * @notice Removes multiple accounts from the banlist at once.
     * @dev This function can only be called by an account with BANLIST_ROLE.
     * @param addresses An array of addresses to be removed from the banlist.
     */
    function unbanAddresses(address[] calldata addresses) external onlyRole(BANLIST_ROLE) {
        for (uint256 i = 0; i < addresses.length; i++) {
            _unbannedAccount(addresses[i]);
        }
    }
    ...
    /**
     * @dev Private function to set the bonus multiplier.
     * @param _bonusMultiplier The new bonus multiplier.
     */
    function _updateBonusMultiplier(uint256 _bonusMultiplier) private {
        if (_bonusMultiplier < _BASE) {
    ...
    /**
     * @notice Sets the bonus multiplier.
    ...
    /**
     * @notice Adds the given amount to the current bonus multiplier.
     * @dev This function can only be called by an account with MULTIPLIER_ROLE.
     * @param _bonusMultiplierIncrement The amount to add to the current bonus multiplier
     */
    function addBonusMultiplier(uint256 _bonusMultiplierIncrement) external onlyRole(MULTIPLIER_ROLE) {
        if (_bonusMultiplierIncrement == 0) {
            revert USDOInvalidBonusMultiplier(_bonusMultiplierIncrement);
        }

        _updateBonusMultiplier(bonusMultiplier + _bonusMultiplierIncrement);
    }
